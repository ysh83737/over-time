...
fn get_config_workbook() -> Result<Xlsx<io::BufReader<fs::File>>, Box<dyn Error>> {
    if fs::metadata(CONFIG_FILE_NAME).is_err() {
        println!(
            "{} {}",
            figures_rs::WARNING.yellow(),
            "未检测到配置文件 config.xlsx".yellow()
        );
        ...
        if result {
            println!("{} {}", figures_rs::INFO.blue(), "生成中...".blue());
            create_config_workbook()?;
            println!("{} {}", figures_rs::TICK.green(), "生成成功！".green());
            println!("{} {}", figures_rs::INFO.magenta(), "请修改配置后重新执行".magenta());
