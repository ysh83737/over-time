...
use inquire;
use rust_xlsxwriter::{
  Workbook as WriterWorkbook,
  XlsxError as WriterError
};
use std::fs;
use std::io;
use std::error::Error;
...
fn get_config_workbook() -> Result<Xlsx<io::BufReader<fs::File>>, Box<dyn Error>> {
  if fs::metadata(CONFIG_FILE_NAME).is_err() {
    ...
    if result {
      ...
      println!("生成成功！");
      println!("请修改配置后重新执行");
      return Err(Box::new(WriterError::CustomError(String::from("请修改配置后重新执行"))));
    } else {
      return Err(Box::new(WriterError::CustomError(String::from("无配置文件"))));
    }
  }
