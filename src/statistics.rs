...
pub fn stat_file(file_path: String, dinner_times: &Vec<TimeRange>) -> Result<(), Box<dyn Error>> {
  ...
  // 获取第一个工作表中的所有行
  let worksheet = workbook.worksheet_range_at(0).ok_or(ReaderError::from("读取工作表失败"))??;

  let rows: Vec<_> = worksheet.rows().collect();
  let header = rows[0];

  // 初始化列索引
  let (mut index_depart, mut index_id, mut index_name, mut index_start, mut index_end, mut index_otype) = (0, 0, 0, 0, 0, 0);
  ...
  for (r_index, row) in rows[1..].iter().enumerate() {
    ...
    // 若员工不存在，则初始化并添加到记录中
    let user = user_records.entry(id.clone())
  ...
  // 写入表头
  stat_sheet.write_row(0, 0, ["员工Id", "部门", "姓名", "节假日加班", "周末加班", "其他加班", "小计"])?;
